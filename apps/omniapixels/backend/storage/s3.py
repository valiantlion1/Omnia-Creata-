...
try:
    ...
    s3_client = minio_client  # Export for health checks
    logging.info(f"MinIO connected: {settings.MINIO_ENDPOINT}")
...
def ensure_bucket_exists():
    ...
    if not minio_client:
        logging.warning("MinIO not available, bucket operations disabled")
        return False
