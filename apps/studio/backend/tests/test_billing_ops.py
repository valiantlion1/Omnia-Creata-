...
async def test_billing_summary_reports_reserved_and_available_credits(
    ...
    try:
        ...
        assert summary["credits"]["reserved_total"] == 2
        assert summary["credits"]["available_to_spend"] == 58
        ...
        assert serialized["reserved_credit_cost"] == 2
        ...
        assert serialized["creative_profile"]["id"] == "internal-fast"
        assert serialized["creative_profile"]["label"] == "FLUX.2 Klein"
        assert serialized["render_experience"]["state"] == "fallback"
        assert draft_guide["quoted_credit_cost"] == 10
        assert draft_guide["reserved_credit_cost"] == 5
        assert draft_guide["settlement_credit_cost"] == 5
        assert draft_guide["creative_profile"]["id"] == "quick-image"
        assert draft_guide["creative_profile"]["badge"] == "Quick 1K"
...
async def test_standard_lane_generation_reserves_and_settles_discounted_credits(
    ...
    try:
        ...
        assert settled.reserved_credit_cost == 2
        assert settled.final_credit_cost == 2
        ...
        assert updated_identity.extra_credits == 58
        assert not any(entry.entry_type == CreditEntryType.TOP_UP for entry in snapshot.credit_ledger.values())
        assert any(
            entry.entry_type == CreditEntryType.GENERATION_SPEND and entry.amount == -2
...
async def test_managed_generation_consumes_monthly_then_extra_credits(
    ...
    try:
        ...
        assert job.reserved_credit_cost == 20
        assert settled.final_credit_cost == 20
...
async def test_failed_generation_releases_reservation_without_spend(
    ...
    try:
        ...
        assert release_entries[0].hold_amount == failed.reserved_credit_cost == 2
...
async def test_billing_summary_surfaces_recent_automatic_credit_reversal_case(
    ...
    try:
        ...
        assert recent_case["hold_amount"] == 2
    finally:
        await service.shutdown()
...
async def test_retryable_failure_keeps_existing_reservation(
    ...
    try:
        ...
        assert retryable.reserved_credit_cost == 2
...
async def test_active_reservation_blocks_over_admission(
    ...
    try:
        ...
        assert first_job.reserved_credit_cost == 2
...
async def test_generation_creation_records_reserve_audit_entry(
    ...
    try:
        ...
        assert reserve_entry.hold_amount == job.reserved_credit_cost == 2
