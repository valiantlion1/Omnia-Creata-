  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#091018_0%,#0c1622_56%,#0a111a_100%)] text-white">
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute left-[8%] top-[6%] h-[22rem] w-[22rem] rounded-full blur-[120px] animate-[oc-gradientShift_20s_ease-in-out_infinite]" style={{ background: 'radial-gradient(circle, rgb(var() / ), transparent 70%)' }} />
        <div className="absolute bottom-[10%] right-[10%] h-[20rem] w-[20rem] rounded-full blur-[130px] animate-[oc-gradientShiftB_24s_ease-in-out_infinite]" style={{ background: 'radial-gradient(circle, rgb(var() / ), transparent 70%)' }} />
        <div className="absolute top-[50%] left-[50%] h-[16rem] w-[16rem] rounded-full blur-[100px]" style={{ background: 'radial-gradient(circle, rgb(var() / ), transparent 70%)' }} />
      </div>
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-6xl flex-col px-5 py-8 md:px-8">
        ...
        <div className="grid flex-1 items-center gap-12 py-12 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-4 text-4xl font-semibold tracking-[-0.05em] text-white md:text-6xl md:leading-[1.02]">Pick up where you <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(90deg, rgb(var(--primary-light)), rgb(var(--accent)))' }}>left off</span>.</h1>
          ...
          <form className="space-y-5 rounded-[24px] border border-white/[0.08] p-6 md:p-8" onSubmit={handleSubmit} style={{ background: 'linear-gradient(180deg, rgba(14,14,22,0.7) 0%, rgba(10,10,16,0.9) 100%)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}>
            <div className="max-w-sm">
              <button
                ...
                className="flex w-full items-center justify-center gap-3 rounded-[14px] border border-white/[0.12] bg-white/[0.04] px-4 py-3 text-sm font-medium text-white transition-all hover:bg-white/[0.08] hover:border-white/[0.18] hover:shadow-[0_0_16px_rgb(var() / )] disabled:cursor-not-allowed disabled:opacity-50"
              >
